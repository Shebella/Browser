    ...
    $.fn.itriFileBrowser = function(settings){
       ...
       function dragHelper(){
            ...
            this.onDrop = function(){
              ...
              if(selections.length > 0) {
				if(selections.length > 1) {
 				   	confirmText = "Move " + selections.length + " items? \r\n";
 				   	confirmText = confirmText + "Are you sure you want to move " + selections.length + " items to " + $(this).find('.name').text() + "?";
 			   	} else {
 			   		confirmText = "Move " + selections.length + " item? \r\n";
			   		confirmText = confirmText + "Are you sure you want to move " + selections.length + " item to " + $(this).find('.name').text() + "?"; 
 			   	}
 			   
				if(confirm(confirmText)) {
					var targetObj = $(this);
					$.each(sel.getDataByIdx(selections), function(){
		        	   	var data = $(this)[0];		        	   	
		        	   	var putType = 'dir';
	               		if(data.type != '0'){
	               			putType = 'obj';	
	                	...
	                	$.ajax({
		                    url: './json/move/' + putType + '/' + data.objectName + '/' + targetObj.find('.name').text() + '/' + settings.bucketName + '' + settings.folderStart + '/' + targetObj.find('.name').text(),
		                    dataType: 'json',
		                    type: 'PUT',
            ...
            this.onDropDone = function(target){
            	if(selections.length > 1) {
            		$.jGrowl("Moved " + selections.length + " items to folder '" + target.find('.name').text() + "'");
            	} else {
            		$.jGrowl("Moved " + sel.getDataByIdx(selections)[0].objectName + " to folder '" + target.find('.name').text() + "'");
            	}
       ...
       this.addFolder = function(param){
          ...
          var $td = $("<td class='nameArea'><img src='"+_param.img+"'/><span class='type'>1_</span><div class='name'><a class='editAble' href='#'>"+ param.name+" ("+l+")" +"</a></div></td>");
          $tr.append($td);
          $tr.append("<td>--</td>");
          $tr.append("<td>--</td>");
          $thisObj.prepend($tr);
         ...
         $editObj = $tr.editAble({
            ...
            onCompleted: function(obj, text){
               ...
               if($tr!=null){//esc cancel
                ...
                var folderStart = _param.folderStart;
                if(folderStart != '/') {
                ...
                $.ajax({
                    url: './json/put/dir/' + _param.bucketName + '' + folderStart + text,
                    dataType: 'json',
                    type: 'PUT',
                    success: function(data, textStatus, jqXHR){
                    	  if(null == data) {
                    		  _param.callback(text, 'error');
                    		  $tr.empty().remove();
                              $tr=null;
                    	  } else {
	                          var res = "";
	                          ...
	                          _param.callback(text, 'success');
	                          $a = $tr.find('a');
	                          $a.attr('href', 'files?bucket=' + _param.bucketName + '&folder=' + folderStart + text );
	                          
	                          if('' != data.modifiedDate) {
	                        	  $($tr.find('td')[2]).text(data.modifiedDate);
	                          }
	                          
	                          $a.removeAttr('style');
	                          
	                          _settings.rawData.push(data);
                    	  }
                     },
                     error : function(jqXHR, textStatus, errorThrown){
                ...
                var data = _sel.getDataByIdx([obj]);
                    if(data.length == 1)
                    {
                    	...
                    	if(data[0].type != '0'){
                     	...
                     	// ajax call to rename
                     	$.ajax({
                         url: './json/rename/' + putType + '/' + data[0].objectName + '/' + text + '/' + _settings.bucketName + '' + _settings.folderStart + '/' + text,
                         dataType: 'json',
                         type: 'PUT',
                         success: function(res_data, textStatus, jqXHR){                       	  
                        	 if(null == res_data) {
                        		 $a.text(data[0].objectName);
                        		 _param.callback(selAry[0], data[0], text, 'error');
								 _settings.onRename(selAry[0], data[0], text, 'error');
	                         } else {
	                        	 $a.prop('href', '../' + res_data.clickAction);
	                        	 _param.callback(selAry[0], data[0], text, 'success');
								 _settings.onRename(selAry[0], data[0], text, 'success');
	                         }
                         },
                         error : function(jqXHR, textStatus, errorThrown){
                              _settings.error(jqXHR, textStatus, errorThrown);
       ...
       this.delObj = function(param){
          ...
          // $childList.unbind();
           ...
           if(selAry.length>0){
        	   var confirmText = "";
			   if(selAry.length > 1) {
				   confirmText = "Delete " + selAry.length + " items? \r\n";
				   confirmText = confirmText + "Are you sure you want to delete " + selAry.length + " items from your Safebox?";
			   } else {
				   var delObj = _sel.getDataByIdx(selAry)[0];
				   var objName = '';
				   if(delObj.type == '0') {
					   confirmText = "Delete folder? \r\n";
					   objName = $(selAry[0]).find('a').text();
				   } else {
					   confirmText = "Delete file? \r\n";
					   objName = delObj.objectName;
				   }				   
				   confirmText = confirmText + "Are you sure you want to delete '" + objName +"' from your Safebox?"; 
			   }
			   ...
			   if(confirm(confirmText)) {
				   ...
				   var delCount = 0;
				   var checkSize = selAry.length;
		           $.each(selAry, function(){
		        	   	var objName = $(this).find('a').text();
		        	   	$.each(_settings.rawData, function() {
		        	   		var delObj = $(this)[0];
		        	   		if(objName == delObj.objectName) {
		        	   			var putType = 'dir';
			               		if(delObj.type != '0'){
			               			putType = 'obj';
			               		}
			                	...
			                	$.ajax({
				                    url: './json/delete/' + putType + '/' + delObj.objectName + '/' + _param.bucketName + '' + _param.folderStart + '/' + delObj.objectName,
				                    dataType: 'json',
				                    type: 'DELETE',
				                    success: function(res_data, textStatus, jqXHR){
				                    	delCount++;
				                    ...
				                    error : function(jqXHR, textStatus, errorThrown){
				                     ...
				                     complete : function(jqXHR, textStatus){
				                    	 if(delCount >= checkSize) {
				                    		 callbackText = "Item was deleted";
				                    		 if(selAry.length > 1) {
				                    			 callbackText = "Items were deleted";
				                    		 }
				                    		 _param.callback(callbackText);
				                    	 }
				                    }
			                	});
		        	   		}
       ...
       this.download = function(param) {
    	 var selAry = _sel.getSelections();
    	 if(1 == selAry.length){
    		 $tr = $("#"+ selAry[0].id); 
    		 $spanType = $tr.find('span');
             $a = $tr.find('a');
             if($spanType.text() != '1') {
