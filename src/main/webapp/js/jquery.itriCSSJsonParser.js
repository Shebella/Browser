      ...
      this.getImgSrc = function(){
       ...
       function buildListItem(itemObj, i){
           ...
           if(_settings.typeAry[itemObj.type] == "folder"){
             ...
             nameExtension = parseNameExtension(itemObj.objectName);
           }
           ...
           var actObj = $('<a href="#"></a>');
           actObj.html(itemObj.objectName);
           actObj.attr('href', '../'+itemObj.clickAction);
           nameObj.append(actObj);
           nametd.append(nameObj);
		   
		   nameObj.attr("title", itemObj.objectName);
		   
           trObj.append(nametd);
           //Size
           var sizetd = $(tdStr);
           //sizetd.html(diskSizeRenderer(itemObj.size))
           if(-1 == itemObj.objectSize) {
        	   sizetd.html("--");
           } else {
        	   sizetd.html(itemObj.objectSize);
           }
           trObj.append(sizetd);
            
           //LastModify
           var lmtd = $(tdStr);
           lmtd.html(itemObj.modifiedDate);
       ...
       function buildFileBrowser(jsonObj){
        ...
        if (null != jsonObj) {
        	var objList = jsonObj.objBeanList;
        	
        	if (null != objList) {
        		for(var i = 0; i < objList.length; i++) {
        			var trObj = buildListItem(objList[i], i);
        			tBody.append(trObj);
        		}
        	}
       	}
